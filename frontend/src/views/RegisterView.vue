<template>
  <section class="min-h-screen flex items-center justify-center bg-gradient-to-b from-sky-50 to-white">
    <div class="bg-white shadow-lg rounded-2xl p-8 w-full max-w-md border border-sky-100">
      <h1 class="text-2xl font-bold text-sky-800 mb-6 text-center">Реєстрація 🧍‍♀️</h1>

      <form @submit.prevent="register" class="flex flex-col gap-4">
        <input v-model="name" type="text" placeholder="Ім’я" required class="border border-sky-300 rounded-lg px-4 py-2" />
        ...
        <input v-model="password_confirmation" type="password" placeholder="Підтверди пароль" required class="border border-sky-300 rounded-lg px-4 py-2" />
      ...
      <p class="text-sky-700 text-center mt-4">
        Уже маєш акаунт?
        <RouterLink to="/login" class="text-sky-600 hover:underline">Увійти</RouterLink>
      </p>
